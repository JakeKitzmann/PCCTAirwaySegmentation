...
def tree_parse():
    ...
    #ids = 10
        img_nib = nibabel.load(os.path.join(input_path, file_list[2*ids]))
        affine = img_nib.affine
        img = img_nib.get_fdata(dtype=np.float32)
        label_nib = nibabel.load(os.path.join(input_path, file_list[2*ids+1]))
        label = label_nib.get_fdata(dtype=np.float32)
        ...
        while whether_refinement(parent_map, children_map, tree_parsing, num, trachea) is True:
            ...
            ad_matric = adjacent_map(tree_parsing, num)
            parent_map, children_map, generation = parent_children_map(ad_matric, trachea, num)
        stem = file_list[2*ids]
        for ext in (".nii.gz", ".nii"):
            if stem.endswith(ext):
                stem = stem[:-len(ext)]
                break
        print(ids, stem, "finished!")
        #tree_parsing[tree_parsing==trachea] = 1

        save_name_img = os.path.join(save_path, stem + "_img.nii.gz")
        save_name_parse = os.path.join(save_path, stem + "_parse.nii.gz")
        save_name_skel = os.path.join(save_path, stem + "_skel.nii.gz")
        save_name_label = os.path.join(save_path, stem + "_label.nii.gz")
        nibabel.save(nibabel.Nifti1Image(img, affine), save_name_img)
        nibabel.save(nibabel.Nifti1Image(tree_parsing.astype(np.uint16), affine), save_name_parse)
        nibabel.save(nibabel.Nifti1Image(skeleton.astype(np.uint8), affine), save_name_skel)
        nibabel.save(nibabel.Nifti1Image(label, affine), save_name_label)
        
        save_name_pm = os.path.join(save_path, stem + "_parent.npy")
        save_name_cm = os.path.join(save_path, stem + "_children.npy")
